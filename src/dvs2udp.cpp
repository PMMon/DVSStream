        ...
        if (packet->getEventType() == POLARITY_EVENT) {
            ...
            for (const auto &evt : *polarity) {
                if (evt.isValid() == true){
                    ...
                    if (include_timestamp){
                        message[current_event] = (polarity_event.x & 0x7FFF) << 16; // Be aware that for machine-independance it should be: htons(polarity_event.x & 0x7FFF);

                        message[current_event+1] = evt.getTimestamp();
                    }
                    else{
                        message[current_event] = (polarity_event.x | 0x8000) << 16; // Be aware that for machine-independance it should be: htons(polarity_event.x | 0x8000);
                    }

                    if (polarity_event.polarity){
                        message[current_event] |= polarity_event.y | 0x8000; // Be aware that for machine-independance it should be: htons(polarity_event.y | 0x8000);
                    }
                    else{
